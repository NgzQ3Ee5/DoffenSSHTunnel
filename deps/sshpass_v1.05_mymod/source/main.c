...
int handleoutput( int fd )
{
    ...
    static int prevmatch1=0,prevmatch3=0,prevmatch6=0,prevmatch7=0,prevmatch8=0,prevmatch9=0; // If the "password" prompt is repeated, we have the wrong password.
    static int state0, state1, state2, state3, state4, state5, state6, state7, state8, state9;
    static const char compare0[]="ogin as"; // Asking for a username
    static const char compare1[]="assword"; // Asking for a password
    //The authenticity of host 'localhost (127.0.0.1)' can't be established.
    //RSA key fingerprint is 13:f4:5d:60:4d:d8:68:db:72:ca:45:07:76:58:41:b8.
    //Are you sure you want to continue connecting (yes/no)?
    static const char compare2[]="The authenticity of host "; // Asks to authenticate host
    static const char compare3[]="assphrase for key"; //Passphrase for key "rsa-key-20100514-password-protected":
    static const char compare4[]="tore key in cache"; //Store key in cache? (y/n)
    static const char compare5[]="pdate cached key"; //Update cached key? (y/n, Return cancels connection)
    static const char compare6[]="nter challenge response"; //Enter challenge response
    static const char compare7[]="nter personal code"; //Enter personal code
    static const char compare8[]="erification code"; //Verification code:
    static const char compare9[]="nter your received challenge"; //Please enter your received challenge response[65]:

    char buffer[256];

    int numread=read(fd, buffer, sizeof(buffer) );
    //DEBUG TO SEE WHAT IS PRINTED 
    //write(1, buffer, numread);
    //fflush(stdout);

    static int stateColon;
    static const char compareColon[]=":";
    stateColon=match( compareColon, buffer, numread, stateColon );

    // Are we at a username prompt?
    state0=match( compare0, buffer, numread, state0 );
    if( compare0[state0]=='\0' && compareColon[stateColon]=='\0' ) {
    	write(1, buffer, numread); fflush(stdout);
		//printf("MYUID:"); fflush(stdout);
        write_pass( fd );
        state0=0;
    }
    ...
    if( compare1[state1]=='\0' && compareColon[stateColon]=='\0' ) {
		if( !prevmatch1 ) {
			write(1, buffer, numread); fflush(stdout);
			//printf("MYPWD:"); fflush(stdout);
	  	  	write_pass( fd );
	  	  	state1=0;
	  	  	prevmatch1=1;
		} else {
    ...
    if( compare2[state2]=='\0' ) {
    	write(1, buffer, numread); fflush(stdout);
		//printf("MYAUTHOST:"); fflush(stdout);
        write_pass( fd );
        state2=0;
    }
    ...
    if( compare3[state3]=='\0' && compareColon[stateColon]=='\0' ) {
		if( !prevmatch3 ) {
			write(1, buffer, numread); fflush(stdout);
			//printf("MYKEYPWD:"); fflush(stdout);
	  	  	write_pass( fd );
	  	  	state3=0;
	  	  	prevmatch3=1;
		} else {
    ...
    if( compare4[state4]=='\0' ) {
    	write(1, buffer, numread); fflush(stdout);
		//printf("MYSTOREKEY:"); fflush(stdout);
        write_pass( fd );
        state4=0;
    }
    ...
    if( compare5[state5]=='\0' ) {
    	write(1, buffer, numread); fflush(stdout);
		//printf("MYUPDATEKEY:"); fflush(stdout);
        write_pass( fd );
        state5=0;
    }
    
    // Enter challenge response
    state6=match( compare6, buffer, numread, state6 );
    if( compare6[state6]=='\0' ) {
		if( !prevmatch6 ) {
			write(1, buffer, numread); fflush(stdout);
			//printf("MYCHALLENGERESP:"); fflush(stdout);
	  	  	write_pass( fd );
	  	  	state6=0;
	  	  	prevmatch6=1;
		} else {
    ...
    // Enter personal code
    state7=match( compare7, buffer, numread, state7 );
    if( compare7[state7]=='\0' ) {
		if( !prevmatch7 ) {
			write(1, buffer, numread); fflush(stdout);
			//printf("MYPERSONALCODE:"); fflush(stdout);
	  	  	write_pass( fd );
	  	  	state7=0;
	  	  	prevmatch7=1;
		} else {
    ...
    // Verification code:
    state8=match( compare8, buffer, numread, state8 );
    if( compare8[state8]=='\0' && compareColon[stateColon]=='\0' ) {
		if( !prevmatch8 ) {
			write(1, buffer, numread); fflush(stdout);
			//printf("MYPERSONALCODE:"); fflush(stdout);
	  	  	write_pass( fd );
    ...
    // nter your received challenge
    state9=match( compare9, buffer, numread, state9 );
    if( compare9[state9]=='\0' ) {
                if( !prevmatch9 ) {
                        write(1, buffer, numread); fflush(stdout);
                        //printf("MYPERSONALCODE:"); fflush(stdout);
                        write_pass( fd );
