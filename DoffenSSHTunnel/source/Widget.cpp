...
void Widget::wireSignals()
{
	...
	for(int i=0;i<stackedWidgetList.size();i++) {
		...
		if(spyOnIt) {
			ATVERIFY( connect( stackedWidgetList.at(i), SIGNAL( currentChanged(int) ), this, SLOT( slotChildWidgetModified() ) ) );
		}
	}
	...
	for(int i=0;i<lineEditList.size();i++) {
		...
		if(spyOnIt) {
			ATVERIFY( connect( lineEditList.at(i), SIGNAL( textChanged(const QString&) ), this, SLOT( slotChildWidgetModified() ) ) );
		}
	}
	...
	for(int i=0;i<radioButtonList.size();i++) {
		...
		if(spyOnIt) {
			ATVERIFY( connect( radioButtonList.at(i), SIGNAL( toggled(bool) ), this, SLOT( slotChildWidgetModified() ) ) );
		}
	}
	...
	for(int i=0;i<checkBoxList.size();i++) {
		...
		if(spyOnIt) {
			ATVERIFY( connect( checkBoxList.at(i), SIGNAL( toggled(bool) ), this, SLOT( slotChildWidgetModified() ) ) );
		}
	}
	...
	for(int i=0;i<myComboBoxList.size();i++) {
		...
		if(spyOnIt) {
			ATVERIFY( connect( myComboBoxList.at(i), SIGNAL( signalModified() ), this, SLOT( slotChildWidgetModified() ) ) );
		}
	}
	QList<QPlainTextEdit*> plainTextEditList = findChildren<QPlainTextEdit*>();
	for(int i=0;i<plainTextEditList.size();i++) {
		ATVERIFY( connect( plainTextEditList.at(i), SIGNAL( textChanged() ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QTextEdit*> textEditList = findChildren<QTextEdit*>();
	for(int i=0;i<textEditList.size();i++) {
		ATVERIFY( connect( textEditList.at(i), SIGNAL( textChanged() ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QSpinBox*> spinBoxList = findChildren<QSpinBox*>();
	for(int i=0;i<spinBoxList.size();i++) {
		ATVERIFY( connect( spinBoxList.at(i), SIGNAL( valueChanged(int) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QDoubleSpinBox*> doubleSpinBoxList = findChildren<QDoubleSpinBox*>();
	for(int i=0;i<doubleSpinBoxList.size();i++) {
		ATVERIFY( connect( doubleSpinBoxList.at(i), SIGNAL( valueChanged(double) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QDateTimeEdit*> dateTimeEditList = findChildren<QDateTimeEdit*>();
	for(int i=0;i<dateTimeEditList.size();i++) {
		ATVERIFY( connect( dateTimeEditList.at(i), SIGNAL( dateTimeChanged(const QDateTime&) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QSlider*> sliderList = findChildren<QSlider*>();
	for(int i=0;i<sliderList.size();i++) {
		ATVERIFY( connect( sliderList.at(i), SIGNAL( valueChanged(int) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QDial*> dialList = findChildren<QDial*>();
	for(int i=0;i<dialList.size();i++) {
		ATVERIFY( connect( dialList.at(i), SIGNAL( valueChanged(int) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<QListWidget*> listWidgetList = findChildren<QListWidget*>();
	for(int i=0;i<listWidgetList.size();i++) {
		ATVERIFY( connect( listWidgetList.at(i), SIGNAL( itemChanged(QListWidgetItem*) ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<Widget*> myWidgetList = findChildren<Widget*>();
	for(int i=0;i<myWidgetList.size();i++) {
		ATVERIFY( connect( myWidgetList.at(i), SIGNAL( signalModified() ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<TableWidget*> myTableWidgetList = findChildren<TableWidget*>();
	for(int i=0;i<myTableWidgetList.size();i++) {
		ATVERIFY( connect( myTableWidgetList.at(i), SIGNAL( signalModified() ), this, SLOT( slotChildWidgetModified() ) ) );
	}
	QList<TreeWidget*> myTreeWidgetList = findChildren<TreeWidget*>();
	for(int i=0;i<myTreeWidgetList.size();i++) {
		ATVERIFY( connect( myTreeWidgetList.at(i), SIGNAL( signalModified() ), this, SLOT( slotChildWidgetModified() ) ) );
	}
}
