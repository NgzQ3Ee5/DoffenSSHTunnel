...
void ATSkeletonWindow::addHostsRecursively(const QJsonArray& jsonHosts, QTreeWidgetItem* parentItem, int levelOffset, const QList<QTreeWidgetItem*>& twiToRemoveList, QList<int>& excludePorts) {
    for (const QJsonValue& hostVal : jsonHosts) {
        ...
        // KeepConn logic ( keep connection active )
        ...
        if (ptToKeepConn) {
            ...
            // Keep 'more tunnels' ports ( safe loop thanks to the test further up ptToRemove->isConnectionDetailsEqual(newTunnel) )
            for(int i=0;i<ptToKeepConn->portForwardList.length();i++) {
                newTunnel->portForwardList[i].nLocalPort = ptToKeepConn->portForwardList[i].nLocalPort;
            }
        ...
        connect(act, &QAction::triggered, this, [this, menuItem] {
            ...
            for(const CustomActionTarget &cat: menuItem.customActionTargetList) {
                ...
                if(delayMs == 0 && menuItem.sLabel.compare("MobaXTerm", Qt::CaseInsensitive) == 0 && !ProcessUtils::isProcessRunning("MobaXterm")) {
                    ...
                    delayMs += m_pMainWindow->preferences()->mobaXTermStartupDelaySeconds() * 1000; //Wait additional 3 seconds for MobaXTerm start for the first time
                }
                delayMs += m_pMainWindow->preferences()->mobaXTermNewTabDelaySeconds() * 1000; // 1 second between each execution
            }
        });
    }
}
